...
parlamento_affluenza = parlamento_affluenza %>%
  select(sezione_elettorale = nr_sez, iscritti = iscr_tot, totale_votanti = vot_tot) %>%
  left_join(parlamento %>% select(sezione_elettorale = sezione, totale_voti_validi), by = "sezione_elettorale")
