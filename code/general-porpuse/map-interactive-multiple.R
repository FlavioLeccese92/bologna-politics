...
map_interactive_multiple = function(polygons,
  ...
  names(election_results)[which(names(election_results) %in% c("nome_lista", "nome_candidato"))] = "y"

  var_ids = election_results %>%
     group_by(y) %>%
     summarise(voti_validi_tot = sum(voti_validi), .groups = "drop") %>%
     slice_max(n = n_layers, order_by = voti_validi_tot) %>%
     pull(y)

  markers = tibble(rank = 1:20)

  for(l in 1:length(var_ids)){
    var_id = var_ids[l]

    election_results_var_id =
      election_results %>%
      group_by(sezione_elettorale) %>%
      summarise(freq = round(100*sum(ifelse(y == var_id, voti_validi, 0))/sum(voti_validi), 2),
                .groups = "drop") %>%
    ...
    polygons = polygons %>%
     ...
     rename_with(~var_id, freq)

    markers = markers %>%
      left_join(top_10_var_id, by = "rank") %>%
      rename_with(~var_id, geometry)
  }
  ...
  var_colors = paletteer_d("ggthemes::few_Dark")[1:n_layers]

  # Define bins
  var_bins = list()
  for(i in 1:length(var_ids)){
    var_bins[[i]] = unique(quantile(na.omit(polygons[[var_ids[i]]]), seq(0, 1, 0.2)))
  }
  ...
  for(i in 1:length(var_ids)){
    ...
    temp_palette = colorRampPalette(c("white", var_colors[i]))(n_bins)
    temp_palette_lines = temp_palette[c(2, rep(1, n_bins - 1))]
    var_palettes[[i]] = colorBin(temp_palette, domain = polygons[[var_ids[i]]], bins = var_bins[[i]])
    var_palettes_lines[[i]] = colorBin(temp_palette_lines, domain = polygons[[var_ids[i]]], bins = var_bins[[i]])
  }
  ...
  for(l in 1:length(var_ids)){
    out = out %>%
      add_choropleth(var_id = var_ids[l], var_pal = var_palettes[[l]], var_pal_lines = var_palettes_lines[[l]], polygons = polygons) %>%
      add_legend(var_id = var_ids[l], var_pal = var_palettes[[l]], polygons = polygons) %>%
      add_markers(var_id = var_ids[l], markers = markers, icon = icon)
  }
  out = out %>%
    # Add layers control
    addLayersControl(
    ...
    htmlwidgets::onRender(paste0("
      function(el, x) {
        var initialLegend = '", var_ids[1],"' // Set the initial legend to be displayed by layerId
        var myMap = this;
